...
resource "aws_instance" "managed_nodes" {
  ...
  key_name = "your keyname"  # change with your pem file
  vpc_security_group_ids = [aws_security_group.tf-sec-gr.id]
  iam_instance_profile = "jenkins-project-profile-${var.user}" # we created this with jenkins server, 14-36 arası satırlar
  tags = {
